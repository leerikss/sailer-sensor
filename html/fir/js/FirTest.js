...
function FirTest() 
{
  ...
  google.maps.event.addListener(
    this.map, "click", 
    function(event) { _this.updateJson(event); _this.drawRawPath(); } 
    );
...
FirTest.prototype.drawFirPath = function()
{
  try
  {
    ...
    // Init buffers with first lat/lng value
    var oldLats = this.getArray( coefs.length, lat );
    var oldLngs = this.getArray( coefs.length, lng );
    ...
    for(var i in json)
    {
      ...
      lat = this.getFirValue( lat, oldLats, coefs, this.laCount++ );
      lng = this.getFirValue( lng, oldLngs, coefs, this.loCount++ );
...
FirTest.prototype.getFirValue = function(val, oldVals, coefs, count)
{
  // Add new val to buffer
  oldVals[count] = val;
